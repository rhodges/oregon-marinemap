...
class NonConsumptive(models.Model):
    ...
    def __unicode__(self):
        return "NonConsumptive, created: %s" % (self.creation_date)
    
    def save(self, *args, **kwargs):
        super(NonConsumptive, self).save(*args, **kwargs)
        if self.active and NonConsumptive.objects.filter(active=True).count() > 1:
            # Ensure that any previously active layer is deactivated
            # There can be only one!
            NonConsumptive.objects.filter(active=True).exclude(pk=self.pk).update(active=False)
    '''            
    class Meta:
        permissions = (
