...
def add_footnotes(ws, row):
    ...
    ws.write(row+1, 0, '1. Gross economic revenues (GER) for charter fisheries are estimated based on fieldwork completed by Steinback et al. (2010).', notes_style)
    ws.write(row+2, 0, '2. Estimated economic output (EEO) is a measure of the total direct and indirect economic activity associated with sales or expenditures in a certain sector, multiplier provided by Gentner and Steinback (2008).', notes_style)
    ws.write(row+3, 0, '3. The percent values represent the percentage of gross economic revenue (GER) found in your Area of Interest for a particular port-fishery or state-fishery combination.', notes_style)